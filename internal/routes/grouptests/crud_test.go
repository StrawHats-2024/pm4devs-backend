...
func TestCRUDGroup(t *testing.T) {
	...
	tests := []assert.HandlerTestCase[responseMessage]{
		...
		},
		{
			...
			Auth:   tokenTwo,
			Body:   `{"group_id": 3, "new_group_name": "nup"}`,
			Status: http.StatusUnprocessableEntity,
			Method: http.MethodPatch,
		},
		{
			Name:   "InvalidGroupID/UPDATE",
			Auth:   tokenTwo,
			Body:   `{"group_id": 0, "new_group_name": "newgroup"}`,
			Status: http.StatusUnprocessableEntity,
			Method: http.MethodPatch,
		},
		{
			...
			Auth:   tokenTwo,
			Body:   `{"group_id": 3, "new_group_name": "newgroup"}`,
			Status: http.StatusUnauthorized,
			Method: http.MethodPatch,
		},
		{
